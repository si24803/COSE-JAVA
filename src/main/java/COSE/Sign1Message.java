 ...
 * This provides for a single signature where the signer is normally implicitly
 * known.
 * {@link SignMessage} need to be used in those cases where more than one
 * signature is needed on a content.
 * There is no way to convert a signed message between the two formats.
 * <p>
 * Create a Sign1Message object for a new message, when processing an existing
 * message use Message.DecodeFromBytes to create the Sign1Message object.
 * <p>
...
public class Sign1Message extends SignCommon {
    ...
    /**
     * Create a signed message object for which there will be a leading tag and the
    ...
    public boolean validate(OneKey cnKey) throws CoseException {
        ...
        return validate(cnKey, (String) null);
    }
    ...
    protected void DecodeFromCBORObject(CBORObject messageObject) throws CoseException {
        ...
        if (messageObject.get(3).getType() == CBORType.ByteString)
            rgbSignature = messageObject.get(3).GetByteString();
        else
            throw new CoseException("Invalid Sign1 structure");
    }
     ...
     * @exception CoseException Errors generated by the COSE module
     */
    public void sign(OneKey key, Provider provider) throws CoseException {
        if (rgbContent == null)
        ...
        rgbSignature = computeSignature(obj.EncodeToBytes(), key, provider);
     ...
     * @exception CoseException Errors generated by the COSE module
     */
    public void sign(OneKey key, String provider) throws CoseException {
        if (rgbContent == null)
            throw new CoseException("No Content Specified");
        if (rgbSignature != null)
            return;
     ...
     * 
     * @param cnKey    key to use for validation
     * @param provider the provider to use for crypto operation
     * @return true if the signature validates
     * @throws CoseException Errors generated by the COSE module
     */
    public boolean validate(OneKey cnKey, String provider) throws CoseException {
        CBORObject obj = CBORObject.NewArray();
        obj.Add(contextString);
        if (objProtected.size() > 0)
            obj.Add(rgbProtected);
        ...
        obj.Add(externalData);
        obj.Add(rgbContent);
        return validateSignature(obj.EncodeToBytes(), rgbSignature, cnKey, provider);
    }
     ...
     * @throws CoseException Errors generated by the COSE module
     */
    public boolean validate(OneKey cnKey, Provider provider) throws CoseException {
        CBORObject obj = CBORObject.NewArray();
        obj.Add(contextString);
        if (objProtected.size() > 0)
            obj.Add(rgbProtected);
